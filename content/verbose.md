---
title: verbose
description: random facts about me
permalink: verbose
---
# Verbose

<p align="center">
    <img src="./resources/avatar_banner.png" height="auto" width="auto" style="margin-bottom:0rem;">
</p>

![[inline_debian.svg]]
![[inline_tmux.svg]]
![[inline_neovim.svg]]
![[inline_golang.svg]]
![[inline_c.svg]]
![[inline_cpp.svg]]
![[inline_lua.svg]]
![[inline_python.svg]]
![[inline_markdown.svg]]
![[inline_docker.svg]]
![[inline_kubernetes.svg]]
![[inline_redis.svg]]
![[inline_postgresql.svg]]
![[inline_git.svg]]

## General
- **Full Name:** Anton Martyukov
- **Unique Name:** atennop
- **Height:** 5'11" / 180cm
- **Weight:** 139lbs / 63kg
- **Eye Color:** green
- **Age:** <a id="real-age" class="internal" style=" font-weight: bold; text-align: center; display: inline-block; width: 130px; vertical-align: middle;"></a> y.o.
- **Death Chance:** <a class="internal" style=" font-weight: bold; text-align: center; display: inline-block; width: 160px; vertical-align: middle;"><span id="mortality-counter"></span></a> per year
- **Company**: none _yet_
- **Timezone:** UTC+3
- **City:** Moscow, Russia
- **Favorite Subject in School**: CS (who'd guess!)
- **T-Shirt Size**: L
- **Neovim Config**: [dotfiles/nvim](https://github.com/Atennop1/dotfiles/tree/main/nvim)
- **Marital Status**: _segmentation fault_

## Projects

### In backend

- [[.|blog]] - this website where I share random notes with the world
- Friendly [**Go SDK**](https://github.com/friendly-social/golang-sdk) and [**CLI client**](https://github.com/friendly-social/cli) - my contributions to the [**Friendly**](https://getfriend.ly) project
- [**atedis**](https://github.com/Atennop1/atedis) - my own implementation of Redis server (WIP)

I moved to backend not so early so there is not much to share _yet_.

### In gamedev
- [**Genesis**](https://github.com/Atennop1/Genesis) - my own 2D OOP game engine in C++
- [**ArkanoidWithoutAnEngine**](https://github.com/Atennop1/ArkanoidWithoutAnEngine) - a game built fully from scratch with it
- [**UnrealShooter**](https://github.com/Atennop1/UnrealShooter) - my first project created with Unreal
- [**Relational-Databases-Via-OOP**](https://github.com/Atennop1/Relational-Databases-Via-OOP) - experimental library for working with databases in OOP style
- [**SavannaSprint**](https://github.com/Atennop1/SavannaSprint) - my first released game (2021, Google Play)

These are completed and actually useful or really meaningful projects I am proud to share. 
But there are also a lot of other little stuff in my archived GitHub repos.

## Work Experience

- **BigTech** (someday)
  <br>I wanna move from gamedev on C# / C++ to backend in Go and be hired as Middle engineer soon.
- **Learning and making personal projects** (2021 - present)
  <br>I was learning gamedev, first Unity and then Unreal Engine, and later decided to move to backend ([[content/field_switch.md|here]] is why), so official start of 
  my career is delayed the second time, which obviously makes me sad.

## Languages 

- **English** (Fluent): _everybody speaks that_
- **Russian** (Native): _mother tongue_

## Socials

- ![[inline_telegram.png|13]] **DM:** [**@atennop**](https://t.me/atennop) – _primary_
- ![[inline_telegram.png|13]] **Channel:** [**@atennopmeta**](https://t.me/atennopmeta) – _updates_
- ![[inline_obsidian.png|13]] **Blog:** [[.|index]] – _this website with random notes_
- ![[inline_github.png|13]] **GitHub:** [**Atennop1**](https://github.com/Atennop1) – _we can collab maybe_
- ![[inline_gmail.png|13]] **Email**: [**atennop@gmail.com**](mailto:atennop@gmail.com) – _i reply pretty fast_

## Political Views

I stand for free within society from oppressive restrictions imposed by authority on one's way of life, behavior, or political views. 
I believe that someday it will stop being my dream and become a reality.

<script src="../scripts/counter.js"></script>
